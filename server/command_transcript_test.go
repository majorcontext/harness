...
func coldThreeCommandFixture(t *testing.T) (h *harness, id string, c0, c1, c2 message.CommandRecord) {
	t.Helper()
	dir := t.TempDir()
	...
	return h, sess.ID, c0, c1, c2
}

func TestTranscriptBootstrapCarriesCommands(t *testing.T) {
	cases := []struct {
		name  string
		setup func(t *testing.T) (h *harness, id string, c0, c1, c2 message.CommandRecord)
	}{
		{"resident", func(t *testing.T) (*harness, string, message.CommandRecord, message.CommandRecord, message.CommandRecord) {
			...
			c0 := newTestCommandRecord("status") // recorded before any message exists: anchor ""
			if err := sess.RecordCommand(c0); err != nil {
				t.Fatalf("RecordCommand c0: %v", err)
			}
			...
			return h, id, c0, c1, c2
		}},
		{"cold", coldThreeCommandFixture},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			h, id, c0, c1, c2 := tc.setup(t)
			...
			if tc.name == "cold" && h.srv.residentSession(id) != nil {
...
func TestMessagePageCarriesCommands(t *testing.T) {
	h, id, c0, c1, c2 := coldThreeCommandFixture(t)

	older := getPageCommands(t, h, id, "?before_seq=5&limit=5") // pages m1..m4
	...
	newest := getPageCommands(t, h, id, "?before_seq=0&limit=3") // pages m6..m8
