...
Keep every STABLE system segment byte-stable within a session; it is part
of the prompt-cache prefix. Render a segment that depends on live state once
and freeze it, as `mcpInstructionsSegment` does. Put text that must change
in the ambient status channel instead. The deferred-MCP catalog is the one
deliberate exception: `select` loads a deferred tool, so
`mcpCatalogSegment` recomputes by design.
