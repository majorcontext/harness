...
var snapshotExcludedSessionFields = map[string]string{
	...
	"ambientSkills":             "per-Prompt remote catalog snapshot; a loaded session begins a fresh Prompt and discovers it again",
	"ambientSkillsSeg":          "per-Prompt remote catalog rendering, rebuilt with ambientSkills",
	"delegatedAmbientSkills":    "last catalog added to a live Claude Code input; a loaded session safely sends its current catalog again",
