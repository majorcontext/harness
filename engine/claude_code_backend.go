...
func (s *Session) consumeClaudeCodeStream(r io.Reader, model message.ModelRef) (finalMsg *message.Message, started bool, turnErr error, zeroMessageOK bool) {
	...
	// pendingReasoningUpstream is this buffer's resolved id, reused by
	// flushPendingReasoning so a streamed delta's id always matches it.
		...
		case "assistant":
			...
			if len(pendingReasoning) > 0 {
				if env.ParentToolUseID == pendingReasoningParent &&
					...
					// pendingReasoningUpstream; this envelope's own
					// mint would leave the two disagreeing.
			...
			if reasoningOnlyParts(msg.Parts) {
				...
				pendingReasoningUpstream = msg.ID
...
type claudeCodeMessage struct {
	...
	// consumeClaudeCodeStream groups on; claudeCodeAssistantMessage adopts
