...
func (s *Session) delegatedAmbientMCPSourceSegments() []string {
	s.mu.Lock()
	defer s.mu.Unlock()
	keys := make([]string, 0, len(s.ambientMCPSources))
	...
	for _, key := range keys {
		snapshot := s.ambientMCPSources[key]
		hash := snapshot.catalog.Hash
		if snapshot.unavailable {
			hash = "unavailable"
		}
		if s.delegatedAmbientMCPSourceHashes[key] == hash {
			continue
		}
		source := s.cfg.AmbientMCPSources[key]
		label := source.Label
		...
		out = append(out, renderAmbientMCPCatalog(key, label, snapshot))
