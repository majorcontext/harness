...
func TestEnqueuePromptPersistsAndEmits(t *testing.T) {
	...
	id, _, err := s.EnqueuePrompt("do the thing", "", PromptProvenance{})
	if err != nil {
...
func TestDequeueFIFOAndJournalsReason(t *testing.T) {
	...
	id1, _, err := s.EnqueuePrompt("first", "", PromptProvenance{})
	if err != nil {
		t.Fatal(err)
	}
	id2, _, err := s.EnqueuePrompt("second", "", PromptProvenance{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueuedPromptsAbsentFromHistory(t *testing.T) {
	...
	if _, _, err := s.EnqueuePrompt("queued text should never appear", "", PromptProvenance{}); err != nil {
		t.Fatal(err)
	}
...
func TestLoadSessionRefoldsQueue(t *testing.T) {
	...
	if _, _, err := s.EnqueuePrompt("a", "", PromptProvenance{}); err != nil {
		t.Fatal(err)
	}
	id2, _, err := s.EnqueuePrompt("b", "", PromptProvenance{})
	if err != nil {
		t.Fatal(err)
	}
	id3, _, err := s.EnqueuePrompt("c", "", PromptProvenance{})
	if err != nil {
		t.Fatal(err)
	}
	if _, _, ok := s.DequeuePrompt("delivered"); !ok {
	...
	id4, _, err := loaded.EnqueuePrompt("d", "", PromptProvenance{})
	if err != nil {
		t.Fatal(err)
	}
	if id4 != id3+1 {
