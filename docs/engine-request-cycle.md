...
- **Model refs are `provider/model`** plus user-defined aliases (`fast`, `smart`) from config. Context-window metadata comes from the curated static `modelmeta` table; the opt-in `context_window_models_dev` source (a control-plane-served snapshot, `docs/design/context-compaction.md`) adds a second lookup for models the table does not carry.
