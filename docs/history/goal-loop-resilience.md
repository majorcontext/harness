...
This document records the implementation history. See `docs/goal-loop.md` for
the current goal-loop contract.
...
arriving mid-wait cuts the schedule short) — per the root `AGENTS.md`, timer-dependent
...
tests (both run inside a `testing/synctest` bubble — no real sleeps, per the
root `AGENTS.md`).
...
**entire** outage — a prompt queued during that outage (see
`docs/session-storage-and-queue.md`) could only ever be injected mid-turn into a doomed
...
(`maybeAutoArmGoal`, upstream of `engine` — see
`docs/session-storage-and-queue.md`) re-enters the loop with a fresh `PursueGoal` call the next time any
