...
func dataURLDims(t *testing.T, dataURL string) (int, int) {
	...
	if i < 0 {
		t.Fatalf("not a base64 data URL: %q", dataURL[:min(40, len(dataURL))])
	}
...
// image in a stored history is downscaled at transcode time, so the request
// the Responses API sees is within the dimension cap rather than 400ing.
func TestTranscodePoisonedHistoryClampsOversizedImage(t *testing.T) {
	...
	for _, c := range p.Content {
		if c.Type == "input_image" && strings.HasPrefix(c.ImageURL, "data:image/") {
			found = true
			w, h := dataURLDims(t, c.ImageURL)
			if w > 7680 || h > 7680 {
