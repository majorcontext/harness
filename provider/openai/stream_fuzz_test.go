...
// model fields are unexported but reachable from within the package, so this
// drives the real readSSE+handle loop with zero per-exec I/O overhead.
// (Preferred per the plan's Task 2: a direct reader-level entry exists here,
// so httptest is unnecessary.)
func newFuzzStream(data []byte) *stream {
	return &stream{
		body:  io.NopCloser(bytes.NewReader(nil)),
		r:     bufio.NewReader(bytes.NewReader(data)),
		model: message.ModelRef{Provider: Family, Model: "fuzz-model"},
	}
}
...
func FuzzStreamDecode(f *testing.F) {
	...
	// Truncated mid-stream: cuts the happy path off partway through an item.
	f.Add([]byte(streamFixture[:len(streamFixture)/2]))
	// Interleaved/malformed: a response.created immediately followed by a
	// failed-response error event, mirroring TestStreamInlineError.
	f.Add([]byte(sse("response.created", `{"type":"response.created","response":{"id":"resp_3"}}`) +
		sse("response.failed", `{"type":"response.failed","response":{"error":{"code":"server_error","message":"boom"}}}`)))
	// Empty input.
	f.Add([]byte(``))
