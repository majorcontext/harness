...
func FuzzStreamDecode(f *testing.F) {
	...
	// Truncated mid-stream: cuts the happy path off partway through a block.
	f.Add([]byte(streamFixture[:len(streamFixture)/2]))
	// Interleaved/malformed: a message_start immediately followed by an
	// inline error event, mirroring TestStreamInlineError.
	f.Add([]byte(sse("message_start", `{"type":"message_start","message":{"id":"msg_02","usage":{"input_tokens":1}}}`) +
